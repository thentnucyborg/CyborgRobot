...
class Controller():
    ...
    def __init__(self):
        ...
        #make a publisher to publish true on the same topic as above. Then publish at the start of controller_init (only for testing, remove/comment out when done)
        
        rospy.loginfo("Remember cyborg_controller will not start running before receiving 'true' on the topic /cyborg_modeselector/behaviour_state. This also means smach_viewer has no input to show.")
    ...
    def controller_init(self, message):
        if message.data == True:
            ...
            path = homedir + "/catkin_ws/src/cyborg_controller/controller.db"
